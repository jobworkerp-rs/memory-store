...
/// 1:1 to the env vars documented in `dot.env` and §P5 of
/// `docs/improve-search-spec.md`. Defaults are deliberately conservative
/// (silent over-fetch is not allowed): callers that hit a hard limit get
/// a `failed_precondition` so the UI can surface the offending size
/// instead of returning a quietly truncated result.
#[derive(Clone, Debug)]
