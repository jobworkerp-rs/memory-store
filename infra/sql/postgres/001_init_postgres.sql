...
--     1. Apply `sql/postgres/manual/005_phase4_drop_thread_id.sql` to
...
-- UNIQUE excludes NULL on both engines, so multiple url rows (sha256 NULL)
-- coexist. Callers MUST use NULL, not '' (empty string collides on UNIQUE).
...
-- MemoryApp::delete_memory) to keep DDL portable. See app/src/app/ for delete logic.
