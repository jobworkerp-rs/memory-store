...
impl MemoryVectorRepositoryImpl {
    ...
    /// returns the total row count of the table (used by the cross-user
    /// header count described in `docs/improve-search-spec.md` §P2).
...
mod test {
    ...
    /// `$LANCE_LANGUAGE_MODEL_HOME/lindera/ipadic/config.yml` (see
    /// `memories/docs/lindera-testing.md`). Without the dictionary this
    /// test would fail in CI, so we exclude it from the default run.
    ///
    /// Run manually with:
    /// ```bash
