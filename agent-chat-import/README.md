...
## Canonical schema (PR5b 以降)

仕様 `docs/agent-chat-import-subcommand-spec.md` §4.2.2 の正規化レイヤを実装し、表示層 / RAG / summary workflow が **source 非依存** で memory metadata を読めるようになった。具体的には:
...
- `metadata.attachment` (image / audio / video / url / ref): `kind`、`storage` (`inline_base64` / `url` / `ref` / `elided` / `invalid`)、`media_type`、`size_bytes`、`data_sha256` (退避時)、`invalid_reason` (dispatch 失敗時)
...
- `metadata.claude_code.*` (claude-code source 限定): JSONL 行の付帯 top-level scalar を **網羅的に転記** したオブジェクト。表示層 (agent-app 等) が本文を読まずに足場情報を判定できる documented contract。
  ...
  - **promoted top-level キー** (`uuid` / `parent_uuid` / `entry_type` / `is_sidechain` / `request_id` / `block_type` / `subtype`) は従来どおり metadata 直下に置かれ、`claude_code.*` には重複させない。
  - codex source は session_meta 付帯 field を従来どおり metadata 直下 (`session_source` / `git` / `cli_version` 等) に持つ。`claude_code` は claude 固有名のため衝突しない。
...
- `MEMORY_ATTACHMENT_INLINE_MAX_BYTES = 1048576` (inline base64 退避閾値)

### 既存 memory の再 import が必要

`claude-code` 経路は **canonical block 分解 (1 message → N memory)** に切り替わったため、`external_id` 形式が `claude_code:<session>:<uuid>` から `claude_code:<session>:<uuid>:b<idx>:<sha1>` に変わった。旧形式と新形式は dedup で衝突しないので、**旧 memory を削除してから再 import** が必須:

```sql
-- claude-code 経由で取り込み済み memory + thread をすべて削除
DELETE FROM thread_label WHERE thread_id IN (SELECT id FROM thread WHERE channel LIKE 'claude_code:%');
DELETE FROM thread_memory WHERE thread_id IN (SELECT id FROM thread WHERE channel LIKE 'claude_code:%');
DELETE FROM memory       WHERE external_id LIKE 'claude_code:%';
DELETE FROM thread       WHERE channel LIKE 'claude_code:%';

-- codex も同等手順 (prefix を 'codex:%' に置き換え)
```

その後 `memories-import claude-code --all-projects` / `memories-import codex --all-sessions` を再実行する。LanceDB を使っている場合は `redispatch_embeddings_for_results` が再 import 時に該当 thread の embedding を再投入するので追加対応不要。

`plain` source は `entry_uid` 形式が変わっていないため再 import 不要。

block 分解により以下の追加 memory が生成されるようになった (旧 legacy 経路では失われていたデータ):
...
旧 `--include-types user,assistant,system` を渡していた既存スクリプトは、canonical kind (`tool_call,tool_output,reasoning,attachment`) を取り込むかどうか改めて判断してください。既定値 (`user,assistant,tool_call,tool_output,system,reasoning,attachment`) は新 schema が定義する全 kind を含む。

## 旧 CLI 形式は廃止

サブコマンド導入に伴い、旧形式 `memories-import -u 1 --all-projects` は **このリリースで動作しなくなった**。

```bash
# OLD (no longer works — clap returns "subcommand required")
...
> **2026-05-05 — gRPC 移行 Phase 1**: importer の DB 直接書き込みを廃止し、すべての書き込みを `memories` gRPC サーバ (`grpc-admin`) 経由に統一しました。
>
> - 必須フラグ: `--server-url <http(s)://host:port>` (`--dry-run` 単独実行時は不要)
> - `--skip-embeddings` フラグは廃止
> - embedding 投入は `AddMemoriesBatch` の commit 後に grpc-admin 側で自動 dispatch されます (LanceDB スタックは必須依存に昇格しているため、サーバが auto-embedding を有効化していれば常時動作します)
> - `SQLITE_URL` / `POSTGRES_URL` / `MEMORY_GRPC_HOST` / `MEMORY_GRPC_PORT` を importer から指定する必要は無くなりました
> - postgres feature, vectordb feature は importer 側から削除
>
> 詳細仕様: [docs/agent-chat-import-grpc-migration-spec.md](../docs/agent-chat-import-grpc-migration-spec.md)
...
# postgres バックエンド (summarize-after / personality-after を OFF)
cargo build --release -p agent-chat-import --no-default-features --features postgres
...
| 変数 | いつ必要か | 例 |
|---|---|---|
| `SQLITE_URL` | DB 書き込み実行時 (= `--dry-run` 以外)。postgres feature 時は対応する `DATABASE_URL` 等を `dot.env` に倣って指定 | `sqlite://test.sqlite3` |
| `MEMORY_GRPC_HOST` / `MEMORY_GRPC_PORT` | embedding redispatch を行うとき (= `--skip-embeddings` を付けない、かつ `memories_imported > 0`) | `localhost` / `9010` |
...
MEMORY_GRPC_HOST=localhost MEMORY_GRPC_PORT=9010 \
...
### `~/.claude/projects/` 配下を全部 + 直近以降のみ + ラベル付与 + embedding skip
...
(複数マッチ時は最長を採用、マッチしなければ従来どおり絶対パスが入る)。
...
例: `cwd=/home/me/work/foo` → ラベルは `path:work/foo` (従来は `path:/home/me/work/foo`)。
...
旧運用で workspace root を指していた場合は、末尾に `/agent-chat-import` を追加する:
...
`user_id` / `updated_after_ms` は memories-import の引数 (`--user-id` と `--since`) から自動的に上書きされる。`--since` 未指定なら `updated_after_ms` は touch しない (テンプレ側の値を尊重)。`--since` を絶対値 (epoch ms) として workflow にそのまま渡すため、jobworkerp のキュー待ち時間が長くても要約対象の下限が import 範囲とぴったり揃う (旧実装の `updated_within_hours` 経路は workflow 起動時刻を起点に再計算するため、ディスパッチ遅延で下限がずれて先頭付近の thread が要約から漏れる問題があった)。
...
仕様: `docs/agent-chat-import-incremental-spec.md` §1。
...
仕様: `ai-docs/user-personality-extraction-spec.md`、ワークフロー詳細は `agent-chat-import/workflows/personality/README.md`。
...
# 単一 rollout
SQLITE_URL=sqlite://test.sqlite3 \
memories-import --user-id 1 codex \
...
既定で `function_call_output` (および `custom_tool_call_output`) は同一 `call_id` を持つ `function_call` の memory を `parent_ids` に張る。Phase 2 のメインループで前方参照しつつ、ループ後に未解決 output を再走査して埋めるので、output が call より前に現れる reorder/編集済み rollout でも親子リンクが復旧する。
...
| `--exclude-encrypted-reasoning` | — | (deprecated) | 旧フラグ。既定で除外されるようになったため no-op。後方互換のために受理されるが新規スクリプトでは不要 |
...
##### `--prune-missing` (Phase A)

`docs/plain-prune-spec.md` で定義された **削除追跡** モード。 `--source-name:` プレフィックス + `user_id` で server 側 memory 集合を取り、 fs に存在しない `metadata.path` を持つ memory を削除する:
...
- rename 追跡 / git history 連携は Phase B (`docs/plain-git-subcommand-spec.md`) で別サブコマンド (`plain-git`) として実装予定

```bash
# per-file (既定)
SQLITE_URL=sqlite://test.sqlite3 \
memories-import --user-id 1 plain --root ~/Obsidian/Private --source-name obsidian-private
...
| `--prune-missing` | — | OFF | fs から消えたファイルの memory を削除 (`docs/plain-prune-spec.md`)。 確認プロンプトあり |
...
| `--skip-embeddings` | — | `false` | embedding redispatch を抑止 |
...
- `agent-chat-import/workflows/thread-summary/README.md` — thread-summary-batch ワークフローの仕様
- `agent-chat-import/workflows/personality/README.md` — thread-personality-batch / user-personality-merge ワークフローの仕様
- `agent-chat-import/workflows/agent-chat-pipeline/README.md` — import → summary → personality を一気通貫で回すパイプライン
- `ai-docs/user-personality-extraction-spec.md` — 嗜好抽出の設計仕様
- `docs/done/claude-code-import-spec.md` — Claude Code 取り込み処理の設計時点スナップショット
- `docs/done/agent-chat-import-subcommand-spec.md` — サブコマンド化仕様 (codex / plain 含む、現行)
