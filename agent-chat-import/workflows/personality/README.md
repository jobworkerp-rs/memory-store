...
仕様: `ai-docs/user-personality-extraction-spec.md`
...
agent-chat-import          (生のチャットログを memory に登録, user_id=N)
        │
        ▼
thread-personality-single  ([1層] スレッド単位の嗜好シグナル / user_id=200000)
        │  (thread-personality-batch がユーザの全スレッドを順次起動)
        ▼
user-personality-merge     ([2層] ユーザ統合プロファイル / user_id=200000)
        │
        ▼
パーソナライズエージェント     (FindListByCondition で profile を取得)
```

`thread-summary` 系 (user_id=100000) と直交する別系統で、入力会話は同じだが
出力スレッド・ラベル・所有者ユーザがすべて分離されている。
...
`personality_user_id` は `user_id` および `summary_user_id` と異なる値である必要があり、
ワークフロー先頭で 400 エラーとして検証される。
...
| `personality_user_id` | `200000` | 嗜好メモリ所有者 |
| `summary_user_id` | `100000` | 衝突検査用 |
...
特定ユーザの嗜好データを全削除する場合は、`personality_user_id` 全体を消すと
他ユーザのデータも消えるので、**ラベルで絞って scope を明示**して削除する:
